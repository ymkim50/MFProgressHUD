...
public let MFProgressMaxOffset: CGFloat = 1000000.0

private let MFDefaultPadding: CGFloat = 4.0
private let MFDefaultLabelFontSize: CGFloat = 16.0
private let MFDefaultDetailsLabelFontSize: CGFloat = 12.0

public protocol MFProgressHUDDelegate: NSObjectProtocol {
    /// Called after the HUD was fully hidden from the screen.
    func hudWasHidden(_ hud: MFProgressHUD) -> Void
}
...
public class MFProgressHUD: UIView {
    
    // MARK: - Properties
    
    /// The HUD delegate object. Receives HUD state notifications.
    public weak var delegate: MFProgressHUDDelegate? = nil
    ...
    /// MFProgressHUD operation mode. The default is indeterminate.
    public var mode: MFProgressHUDMode = .indeterminate {
        didSet {
    ...
    public /*dynamic*/ var animationType: MFProgressHUDAnimation = .fade
    
    /// The bezel offset relative to the center of the view. You can use MFProgressMaxOffset
    /// and -MFProgressMaxOffset to move the HUD all the way to the screen edge in each direction.
    /// E.g., CGPoint(x: 0.0, y: MFProgressMaxOffset) would position the HUD centered on the bottom edge.
    ...
    public lazy var bezelView: MFBackgroundView = MFBackgroundView(frame: .zero)
    
    /// View covering the entire HUD area, placed behind bezelView.
    public lazy var backgroundView: MFBackgroundView = MFBackgroundView(frame: self.bounds)
    
    /// The UIView (e.g., a UIImageView) to be shown when the HUD is in MFProgressHUDModeCustomView.
    ...
    public lazy var button: UIButton = MFRoundedButton(frame: .zero)
...
#if DEBUG
        print("MFProgressHUD -> release memory.")
#endif
    }
}

extension MFProgressHUD {
    
    // MARK: - Class methods, Show & hide
    
    public class func show(to view: UIView, animated: Bool) -> MFProgressHUD {
        let hud = MFProgressHUD(with: view)
    ...
    public class func hud(for view: UIView) -> MFProgressHUD? {
        let subviewsCollection = view.subviews.reversed()
        for subView in subviewsCollection {
            if subView is MFProgressHUD, let hud = subView as? MFProgressHUD {
    ...
    public func show(animated: Bool) {
        assert(Thread.isMainThread, "MFProgressHUD needs to be accessed on the main thread.")
    ...
    func animate(in animating: Bool, type: MFProgressHUDAnimation, completion: ((Bool) -> Void)?) {
        var type = type
...
extension MFProgressHUD {
    ...
    func setupViews() {
        ...
        label.font = UIFont.boldSystemFont(ofSize: MFDefaultLabelFontSize)
        ...
        detailsLabel.font = UIFont.boldSystemFont(ofSize: MFDefaultDetailsLabelFontSize)
        detailsLabel.isOpaque = false
        detailsLabel.backgroundColor = UIColor.clear
        
        button.titleLabel?.textAlignment = .center
        button.titleLabel?.font = UIFont.boldSystemFont(ofSize: MFDefaultDetailsLabelFontSize)
    ...
    func updateIndicators() {
        ...
        let isRoundIndicator = indicator is MFRoundProgressView
        ...
        case .determinate, .annularDeterminate:
            ...
            if !isRoundIndicator {
                ...
                indicator?.removeFromSuperview()
                let roundView = MFRoundProgressView()
                bezelView.addSubview(roundView)
                indicator = roundView
            }
            
            if mode == .annularDeterminate {
                (indicator as? MFRoundProgressView)?.isAnnular = true
            }
        case .customView:
    ...
    func updateViews(for color: UIColor) {
        ...
        } else if indicator is MFRoundProgressView {
            
            (indicator as? MFRoundProgressView)?.progressTintColor = color
            (indicator as? MFRoundProgressView)?.backgroundTintColor = color.withAlphaComponent(0.1)
        } else if indicator is MFRoundProgressView {
            
            (indicator as? MFBarProgressView)?.progressColor = color
            (indicator as? MFBarProgressView)?.lineColor = color
        } else {
...
extension MFProgressHUD {
    ...
    func updatePaddingConstraints() {
        ...
        for padding in paddingConstraints {
            ...
            padding.constant = (firstVisible && (secondVisible || hasVisibleAncestors)) ? MFDefaultPadding : 0.0
